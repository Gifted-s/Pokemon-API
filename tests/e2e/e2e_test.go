...
func TestPokemonAPI(t *testing.T) {
	...
	t.Run("Test Response Keys", func(t *testing.T) {
		defer db.DropDB()
		fakeRandomPokemons := fixtures.CreateFakePokemons()
		e := pokmonRouteTester(t)
		db.InsertPokemons(fakeRandomPokemons)
	...
	t.Run("Test Response Page Size", func(t *testing.T) {
		defer db.DropDB()
		fakeRandomPokemons := fixtures.CreateFakePokemons()
		e := pokmonRouteTester(t)
		db.InsertPokemons(fakeRandomPokemons)
	...
	t.Run("Test filter with HP", func(t *testing.T) {
	...
	t.Run("Test filter with Defence", func(t *testing.T) {
	...
	t.Run("Test filter with Attack ", func(t *testing.T) {
		defer db.DropDB()
		e := pokmonRouteTester(t)
		fakeId1, _ := primitive.ObjectIDFromHex("62a705c9cfb07e0c463b316d")
		fakeId2, _ := primitive.ObjectIDFromHex("62a705c9cfb07e0c463b3167")
	...
	t.Run("Test response sort based on edit distance", func(t *testing.T) {
	...
	t.Run("Test all filters", func(t *testing.T) {
	...
	t.Run("Test response for invalid query param", func(t *testing.T) {
		defer db.DropDB()
		wrongParam := "hp[wrong_operator]"
		fakeRandomPokemons := fixtures.CreateFakePokemons()
		e := pokmonRouteTester(t)
		db.InsertPokemons(fakeRandomPokemons)
		r := e.GET("/pokemon").
			WithQuery(wrongParam, "100").WithQuery("defense[gte]", "100").WithQuery("attack[gte]", "100").WithQuery("page", "1").WithQuery("search", "Chaldeans").
			Expect().
			Status(http.StatusBadRequest).JSON().Object()

		r.Keys().ContainsOnly("status", "error")
		r.Value("error").Equal("wrong_operator: is not a valid operator please select operator from these [gte | lte | gt | lt | eq | ne ]")
	})
}
